...
public abstract class World implements IBlockAccess {
    ...
    public int getLightFromNeighborsFor(LightType type, BlockPos pos) {
        if (this.provider.getHasNoSky() && type == LightType.SKY) {
            return 0;
        } else {
            if (pos.getY() < 0) {
    ...
    protected void updateWeather() {
        if (!this.provider.getHasNoSky()) {
            if (!this.isRemote) {
                ...
                this.thunderingStrength = Math.clamp(this.thunderingStrength, 0.0F, 1.0F);
                ...
                this.rainingStrength = Math.clamp(this.rainingStrength, 0.0F, 1.0F);
            }
        }
    }
