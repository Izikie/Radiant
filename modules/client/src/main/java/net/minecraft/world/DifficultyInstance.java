...
public class DifficultyInstance {
    ...
    private float calculateAdditionalDifficulty(Difficulty difficulty, long worldTime, long chunkInhabitedTime, float moonPhaseFactor) {
        if (difficulty == Difficulty.PEACEFUL) {
            ...
            float f = 0.75F;
            float f1 = Math.clamp((worldTime - 72000.0F) / 1440000.0F, 0.0F, 1.0F) * 0.25F;
            f = f + f1;
            float f2 = 0.0F;
            f2 = f2 + Math.clamp(chunkInhabitedTime / 3600000.0F, 0.0F, 1.0F) * (flag ? 1.0F : 0.75F);
            f2 = f2 + Math.clamp(moonPhaseFactor * 0.25F, 0.0F, f1);
