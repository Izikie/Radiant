...
### How to compile
1. Ensure a valid C/C++ compiler is installed.
2. Run the `RunClientNativeAgent` Gradle task. It's recommended to load a world first so the tracing agent captures all necessary classes.  
   **ALWAYS DO THIS BEFORE DOING STEP 3!**
3. Run the `nativeCompile` task. Compilation time depends on your system.
...
2. Run the `RunClientNativeAgent` gradle task, it is recommended that you load a world to make sure the tracing agent doesn't miss anything.
   **ALWAYS DO THIS BEFORE DOING STEP 3!**
3. Run the compile script located in the `nativeimage` folder, depending on your OS. Compilation time depends on your system.
